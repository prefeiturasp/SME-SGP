...
namespace MSTech.GestaoEscolar.BLL
{
    ...
    public class CLS_AlunoAvaliacaoTurmaDisciplinaBO : BusinessBase<CLS_AlunoAvaliacaoTurmaDisciplinaDAO, CLS_AlunoAvaliacaoTurmaDisciplina>
    {
        ...
        /// </summary>
        /// <param name="listaTurmaDisciplina">Lista de ids de turma disciplina.</param>
        /// <returns></returns>
        public static List<CLS_AlunoAvaliacaoTurDis_PendenciaEfetivacao> SelecionaPendenciasFechamentoDisciplinas(List<sTurmaDisciplinaEscolaCalendario> listaTurmaDisciplina, Guid ent_id, int appMinutosCacheLongo = 0)
        {
            List<CLS_AlunoAvaliacaoTurDis_PendenciaEfetivacao> dados = new List<CLS_AlunoAvaliacaoTurDis_PendenciaEfetivacao>();
            ...
            if (listaTurmaDisciplina.Any())
            {
                if (appMinutosCacheLongo > 0)
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        using (DataTable dtRetorno = new CLS_AlunoAvaliacaoTurmaDisciplinaDAO().SelecionaPendenciasFechamentoDisciplinas(dt, ACA_ParametroAcademicoBO.ParametroValorInt32PorEntidade(eChaveAcademico.TIPO_EVENTO_EFETIVACAO_NOTAS, ent_id)))
                        {
                            dados.AddRange
                            (
                                dtRetorno.Rows.Cast<DataRow>()
                                              .Select
                                              (
                                                  p =>
                                                      (CLS_AlunoAvaliacaoTurDis_PendenciaEfetivacao)GestaoEscolarUtilBO.DataRowToEntity(p, new CLS_AlunoAvaliacaoTurDis_PendenciaEfetivacao())
                                              ).ToList()
                            );
                            ...
                            var dadosRegencia = from CLS_AlunoAvaliacaoTurDis_PendenciaEfetivacao pend in dados
                    ...
                    dados.AddRange
                    (
                        listaCache.Where(p => p.cacheIsSet && p.tud_tipo != (byte)TurmaDisciplinaTipo.Regencia).Select(p => CacheManager.Factory.Get<CLS_AlunoAvaliacaoTurDis_PendenciaEfetivacao>(String.Format(ModelCache.PENDENCIA_FECHAMENTO_ESCOLA_TURMA_DISCIPLINA_MODEL_KEY, p.esc_id, p.uni_id, p.cal_id, p.tud_id)))
                    );

                    dados.AddRange
                    (
                        listaCache.Where(p => p.cacheIsSet && p.tud_tipo == (byte)TurmaDisciplinaTipo.Regencia).SelectMany(p => CacheManager.Factory.Get<List<CLS_AlunoAvaliacaoTurDis_PendenciaEfetivacao>>(String.Format(ModelCache.PENDENCIA_FECHAMENTO_ESCOLA_TURMA_DISCIPLINA_MODEL_KEY, p.esc_id, p.uni_id, p.cal_id, p.tud_id)))
                    );
                }
                else
                {
                    ...
                    using (DataTable dtRetorno = new CLS_AlunoAvaliacaoTurmaDisciplinaDAO().SelecionaPendenciasFechamentoDisciplinas(dt, ACA_ParametroAcademicoBO.ParametroValorInt32PorEntidade(eChaveAcademico.TIPO_EVENTO_EFETIVACAO_NOTAS, ent_id)))
                    {
                        dados = dtRetorno.Rows.Cast<DataRow>()
                                              .Select
                                              (
                                                  p =>
                                                      (CLS_AlunoAvaliacaoTurDis_PendenciaEfetivacao)GestaoEscolarUtilBO.DataRowToEntity(p, new CLS_AlunoAvaliacaoTurDis_PendenciaEfetivacao())
