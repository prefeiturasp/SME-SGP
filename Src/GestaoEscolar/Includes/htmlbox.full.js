...
$.fn.htmlbox=function(options){
	...
	var camposauxiliares = ['Nome do Aluno','[nomeAluno]','Nome do Pai','[nomePai]','Nome da Mãe','[nomeMae]'];
		...
		for(var k=0;k<d.toolbars.length;k++){
			...
			for(var i=0;i<(toolbar.length);i++){
				...
				// END: Custom button
					...
					h += "</select></td>";
// CAMPOS AUXILIARES
				} else if (toolbar[i] === "campoauxiliar") {
					h += "<td valign='middle' align='center'><select id='" + d.id + "_campoauxiliar' onchange='global_hb[\"" + d.id + "\"].cmd(\"campoauxiliar\",this.options[this.selectedIndex].value)' style='font-size:12px;'><option value='' selected>- CAMPO AUXILIAR -</option>";
					for (var m = 0; m < camposauxiliares.length; m++) {
					    if (m % 2) { continue; }
						    h += "<option value='" + camposauxiliares[m + 1] + "'>" + camposauxiliares[m] + "</option>";
					}
					h += "</select></td>";
// FIM CAMPOS AUXILIARES
				}else if(toolbar[i]==="highlight"){
					h += "<td valign='middle' align='center'><select id='"+d.id+"_highlight' onchange='global_hb[\""+d.id+"\"].cmd(\"backcolor\",this.options[this.selectedIndex].value)' style='font-size:12px;'><option value='' selected>-REALCE-</option>";
				...
				// Commands
				var cmds = {"about":"Sobre","bold":"Negrito","center":"Centralizar","code":"Ver HTML","copy":"Copiar","cut":"Recortar","hr":"Inserir Linha","link":"Inserir Link","image":"Inserir Imagem","indent":"Aumentar Recuo","italic":"Itálico","justify":"Justificar","left":"Alinhar a Esquerda","ol":"Numeração","outdent":"Diminuir Recuo","paragraph":"Inserir Parágrafo","paste":"Colar","quote":"Quote","redo":"Refazer","removeformat":"Remove Format","right":"Alinhar a Direita","strike":"Tachado","striptags":"Strip Tags","sub":"Subscrito","sup":"Sobrescrito","ul":"Marcadores","underline":"Sublinhar","undo":"Desfazer","unlink":"Remover HyperLink"};
