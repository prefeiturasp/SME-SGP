    ...
    $.extend({
        ...
        tablesorter: new function () {
            ...
            ts.defaults = {
                ...
                textSorter: 'simple',       // choose overall or specific column sorter function(a, b, direction, table, columnIndex) [alt: ts.sortText]
            ...
            ts.getElementText = function (c, node, cellIndex) {
                ...
                if (typeof (t) === 'string') {
                    ...
                    return $.trim((t === 'basic' ? $node.attr(c.textAttribute) || node.textContent : node.textContent) || $node.text() || '').latinise();
                } else {
                    if (typeof (t) === 'function') {
                        return $.trim(t($node[0], c.table, cellIndex)).latinise();
                    } else if (typeof (te = ts.getColumnData(c.table, t, cellIndex)) === 'function') {
                        return $.trim(te($node[0], c.table, cellIndex)).latinise();
                    }
                }
                // fallback
                return $.trim($node[0].textContent || $node.text() || '').latinise();
            };
