...
public partial class WebControls_TurmaDisciplina_UCGridDisciplina : MotherUserControl
{
    #region Constantes

    private const int indiceColunaQtAulasSemanais = 1;
    private const int indiceColunaAvaliacoesPeriodicas = 3;
    ...
    protected void gvDisciplina_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            Int16 separaDispEscpecial = Convert.ToInt16(DataBinder.Eval(e.Row.DataItem, "tds_separaDispEspecial"));
            bool disciplinaEspecial = Convert.ToBoolean(DataBinder.Eval(e.Row.DataItem, "tud_disciplinaEspecial"));
            ...
            {
                CheckBox chkAlunoDef = (CheckBox)e.Row.FindControl("chkAlunoDef");
                if (chkAlunoDef != null)
                {
            ...
            // Se a coluna de qt de aulas semanais não estiver visível, não valida o txt.
            RequiredFieldValidator rfvAulaSemanal = (RequiredFieldValidator)e.Row.FindControl("rfvAulaSemanal");
            rfvAulaSemanal.Visible = gvDisciplina.Columns[indiceColunaQtAulasSemanais].Visible;
        }
    }
