...
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[spGrantExectoAllRoutines]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [dbo].[spGrantExectoAllRoutines]
	@user sysname
	, @database sysname
AS
BEGIN
...
EXEC spGrantExectoAllRoutines insira_seu_usuario, GestaoPedagogica
GO
